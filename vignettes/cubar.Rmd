...
head(optimal_codons[optimal_codons$coef < 0 & optimal_codons$qvalue < 0.001, ])
```
...
yeast_leg <- head(yeast_exp[order(yeast_exp$fpkm), ], n = 500)
yeast_leg <- yeast_leg[yeast_leg$gene_id %in% rownames(yeast_cf), ]

# differetial usage test
du_test <- codon_diff(yeast_cds_qc[yeast_heg$gene_id], yeast_cds_qc[yeast_leg$gene_id])
du_test <- du_test[du_test$amino_acid != '*', ]
```
...
```{r}
du_test$codon <- factor(du_test$codon, levels = du_test$codon[order(-du_test$global_or)])

ggplot(du_test, aes(x = codon, y = log2(global_or), fill = factor(du_test$global_padj < 0.05))) +
    geom_col() +
    labs(x = NULL, y = 'log2(OR)') +
    scale_fill_discrete(name = NULL, breaks = c(F, T), labels = c('NS', 'Significant')) +
    theme_classic(base_size = 12) +
    theme(axis.text = element_text(color = 'black'),
...
```{r}
du_test2 <- du_test[!du_test$amino_acid %in% c('Met', 'Trp'), ]
du_test2$codon <- factor(du_test2$codon, levels = du_test2$codon[order(-du_test2$fam_or)])
...
`cubar` provides a function to perform a sliding-window analysis of codon usage bias. This analysis can be useful for identifying regions of a gene that exhibit distinct codon usage patterns. Here, we demonstrate how to perform a sliding-window analysis on the longest yeast gene YLR106C.
