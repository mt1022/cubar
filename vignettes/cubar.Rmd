...
`cubar` provides a method to determine the optimal (or "preferred") codon for each codon subfamily based on regression of codon usage against scores for genes. Preferred codons are more likely to be used in genes with high scores. Consequently, preferred codons will have positive coefficients in the regression analysis. Users can provide their own gene scores, for example, `log1p`-transformed gene expression levels (RPKM or TPM). If no gene scores are provided, `cubar` will use `-ENC` (so that genes with stronger codon usage bias have larger scores) by default.
